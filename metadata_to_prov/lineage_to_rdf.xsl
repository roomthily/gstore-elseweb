    ...
    <xsl:template match="/metadata">
        <rdf:RDF>
            ...
            <xsl:for-each select="$sources/srccite/citeinfo/origin[text() !='']">
                <owl:NamedIndividual rdf:about="{fn:concat('http://gstore.unm.edu/elseweb/', .)}">
            ...
            <xsl:for-each select="$sources">
                <owl:NamedIndividual rdf:about="{fn:concat('http://gstore.unm.edu/elseweb/', srccite/citeinfo/title)}">
                    ...
                    <prov:wasGeneratedBy rdf:resource="{fn:concat('http://gstore.unm.edu/elseweb/', $step-for-srcprod, '#', srccite/citeinfo/title)}"/>
                </owl:NamedIndividual>
            </xsl:for-each>
            ...
            <xsl:for-each select="$steps">
                ...
                <owl:NamedIndividual rdf:about="{fn:concat('http://gstore.unm.edu/elseweb/', $activity)}">
                    ...
                    <xsl:for-each select="$source">
                        <!-- dataset -->
                        <prov:used rdf:resource="{fn:concat('http://gstore.unm.edu/elseweb/', title)}"/>
                        <!-- software agent -->
                        <prov:wasAssociatedWith rdf:resource="{fn:concat('http://gstore.unm.edu/elseweb/', origin)}"/>
                    </xsl:for-each>
                </owl:NamedIndividual>
            </xsl:for-each>
        </rdf:RDF>
