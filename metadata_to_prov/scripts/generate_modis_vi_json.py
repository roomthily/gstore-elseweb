...
FOR AQUA:
http://e4ftl01.cr.usgs.gov/MOLA/MYD13Q1.005
...
FOR TERRA:
http://e4ftl01.cr.usgs.gov/MOLT/MOD13Q1.005/
...
AQUA_ROOT = 'http://e4ftl01.cr.usgs.gov/MOLA/MYD13Q1.005'
TERRA_ROOT = 'http://e4ftl01.cr.usgs.gov/MOLT/MOD13Q1.005/'
...
with open('../../../modis_vi_PRODUCTION_20140626.csv', 'r') as f:
...
for html in htmls:
    ...
    dataset = [d for d in datasets if d[3] == '_'.join([hdfs[0][0], hdfs[0][1][1:], INDEX, 'MOSAIC'])]
    if not dataset or len(dataset) > 1:
        print 'bad dataset', hdfs[0]
        continue
    dataset = dataset[0]
    uuid = dataset[1]
    ...
    for hdf in hdfs:
        ...
        md = {
            ...
            "filename": '_'.join([hdf[0], hdf[1][1:], INDEX]) + '.tif',
            "parameter": "Vegetation",
            "spref": {
            ...
            "end_date": end_date,
            "band": {
                "name": "MODIS_Grid_16DAY_250m_500m_VI:250m 16 days %s" % INDEX,
                "units": "unitless"
            }
        }
        ...
        download_steps.append({
            ...
            "description": "Download MODIS %s Vegetation HDF from LPDAAC" % PLATFORM,
            ...
            "inputs": [
                {
                    "id": "RemoteHDF-%s" % hdf[2],
                    "type": "MODISDataset",
                    "description": "Download link for the MODIS %s Vegetation HDF" % PLATFORM
                }
            ]
        })
        ...
        extract_steps.append({
            ...
            "description": "Extract and reproject the %s from the %s HDF" % (INDEX, PLATFORM),
            "procdate": pubdate + 'T00:00:00',
            "source_produced_id": "LocalTIFF-%s" % hdf[2],
            "source_type": "ExtractedDataset",
            "source_description": "WGS84 GeoTIFF of the %s from the %s HDF" % (INDEX, PLATFORM),
            "software": "gdal_warp",
            "inputs": [
                {
                    "id": "LocalHDF-%s" % hdf[2],
        ...
        mosaic_inputs.append({
            ...
            "description": "WGS84 GeoTIFF of the %s from the %s HDF" % (INDEX, PLATFORM)
        })
    ...
    mds.append({
        "id": "MOSAIC",
        "filename": '_'.join([hdf[0], hdf[1][1:], INDEX, 'MOSAIC']) + '.tif',
        "parameter": "Vegetation",
        "spref": {
            "name": _sprefs[4326]['name'], 
            "url": _sprefs[4326]['url'], 
    ...
    steps.append({
        "id": "mosaic-1",
        "name": "Mosaic",
        "description": "Mosaic the %s tiles into one GeoTIFF" % INDEX,
        "procdate": pubdate + 'T00:00:00',
        "source_produced_id": "MOSAIC",
        "source_type": "MosaicDataset",
        "source_description": "Mosaicked GeoTIFF for the %s band" % INDEX,
        "software": "gdal_merge",
    ...
    steps.append({
        ...
        "source_description": "WCS for the MODIS %s %s dataset" % (PLATFORM, INDEX),
        "software": "GSTORE",
        "inputs": [
            {
                "id": "mosaic-1",
                "type": "MosaicDataset",
                "description": "Mosaicked GeoTIFF for the %s band" % INDEX
            }
        ]   
    })
